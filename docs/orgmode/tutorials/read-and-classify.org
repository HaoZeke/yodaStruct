#+TITLE: Tutorial: Read a dump and classify
...
- This repository, with work from the repository root so the dump
  path resolves
- The [[file:../quickstart.org][Quickstart]] build, or the meson / Nix
  steps in Step 1
...
If you already ran the [[file:../quickstart.org][Quickstart]], the
...
~chill-plus~, is in [[file:../howto/fennel.org][Fennel]].
...
- [[file:../howto/fennel.org][Call dseams from Fennel]] - Lisp syntax on the same table
- [[file:../howto/embed-lua.org][Embed in a Lua host]] - Load the ~.so~ from your own process
- [[file:../reference/lua.org][Lua surface]] - Helpers and ~dseams.core~ names
- [[file:../howto/install.org][Install the library]] - How the ~.so~ is built and found
